  ...
  return (
    <div className="p-8 space-y-8">
      <div>
        ...
        <p className="text-white/50">Pay your funded invoices</p>
      </div>

      {error && (
        <div className="p-4 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-400 flex items-center gap-2">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="bg-white/5 border-white/10 backdrop-blur-md">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-white/70">Total Due</CardTitle>
            <CreditCard className="h-4 w-4 text-white/50" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-white">{totalDue.toFixed(2)} XLM</div>
            <p className="text-xs text-white/40">{settlements.length} invoices</p>
          </CardContent>
        </Card>

        <Card className="bg-white/5 border-white/10 backdrop-blur-md">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-white/70">Overdue</CardTitle>
            <AlertCircle className="h-4 w-4 text-amber-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-amber-400">{overdueCount}</div>
            <p className="text-xs text-white/40">Require immediate attention</p>
          </CardContent>
        </Card>

        <Card className="bg-white/5 border-white/10 backdrop-blur-md">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-white/70">Payment Method</CardTitle>
            <CheckCircle2 className="h-4 w-4 text-emerald-400" />
          </CardHeader>
          <CardContent>
            <div className="text-lg font-bold text-white">Custodial Wallet</div>
            <p className="text-xs text-white/40">No signature required</p>
          </CardContent>
        </Card>
      </div>
      ...
      {settlements.length === 0 ? (
        <Card className="bg-white/5 border-white/10 backdrop-blur-md">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-white/50">No pending settlements.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-6">
          {settlements.map((settlement) => {
            ...
            return (
              <Card
                key={settlement.id}
                className={`bg-white/5 border-white/10 backdrop-blur-md ${isOverdue ? 'border-amber-500/50' : ''}`}
              >
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-white/40">Settlement Amount</div>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-white">{baseAmount.toFixed(2)} XLM</p>
                    </div>
                    <div>
                      <span className="text-white/50">Interest ({settlement.interestRate}%)</span>
                      <p className="font-medium text-white">{interest.toFixed(2)} XLM</p>
                    </div>
                    <div>
